...
Detect::Detect(const void* data, size_t length) {
  ...
  // printf("anchor:%d,classes:%d,gh:%d,gw:%d,size:%d\n", _n_anchor, _n_classes,
  // _n_grid_h, _n_grid_w, _n_output_size);
  assert(d == a + length);
}
...
__global__ void gpu_detect_layer(const float* input_, float* output_,
  ...
  output_[bbindex + numGridCells * (z_id * (5 + n_classes_) + 0)] =
      2.f *
          sigmoidGPU(
              input_[bbindex + numGridCells * (z_id * (5 + n_classes_) + 0)]) -
      0.5f;

  output_[bbindex + numGridCells * (z_id * (5 + n_classes_) + 1)] =
      2.f *
          sigmoidGPU(
              input_[bbindex + numGridCells * (z_id * (5 + n_classes_) + 1)]) -
      0.5f;

  float w =
      2.f * sigmoidGPU(
                input_[bbindex + numGridCells * (z_id * (5 + n_classes_) + 2)]);
  output_[bbindex + numGridCells * (z_id * (5 + n_classes_) + 2)] = w * w;

  float h =
      2.f * sigmoidGPU(
                input_[bbindex + numGridCells * (z_id * (5 + n_classes_) + 3)]);
  output_[bbindex + numGridCells * (z_id * (5 + n_classes_) + 3)] = h * h;

  output_[bbindex + numGridCells * (z_id * (5 + n_classes_) + 4)] = sigmoidGPU(
      input_[bbindex + numGridCells * (z_id * (5 + n_classes_) + 4)]);
  for (uint32_t i = 0; i < n_classes_; ++i) {
    output_[bbindex + numGridCells * (z_id * (5 + n_classes_) + (5 + i))] =
        sigmoidGPU(input_[bbindex +
                          numGridCells * (z_id * (5 + n_classes_) + (5 + i))]);
  }
}

cudaError_t cuda_detect_layer(const void* input_, void* output_,
  ...
  for (int batch = 0; batch < batch_size_; ++batch) {
    gpu_detect_layer<<<number_of_blocks, threads_per_block, 0, stream_>>>(
        ...
        reinterpret_cast<float*>(output_) + (batch * n_output_size_), grid_h_,
        grid_w_, n_classes_, n_anchor_);
  }

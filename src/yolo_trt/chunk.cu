...
size_t Chunk::getWorkspaceSize(int maxBatchSize) const { return 0; }
...
size_t Chunk::getSerializationSize() const { return sizeof(_n_size_split); }
...
bool Chunk::isOutputBroadcastAcrossBatch(int outputIndex,
                                         const bool* inputIsBroadcasted,
                                         int nbInputs) const {
  return false;
}

bool Chunk::canBroadcastInputAcrossBatch(int inputIndex) const { return false; }

void Chunk::attachToContext(cudnnContext* cudnnContext,
                            cublasContext* cublasContext,
                            IGpuAllocator* gpuAllocator) {}

void Chunk::configurePlugin(const PluginTensorDesc* in, int nbInput,
                            const PluginTensorDesc* out, int nbOutput) {
  _n_size_split =
      in->dims.d[0] / 2 * in->dims.d[1] * in->dims.d[2] * sizeof(float);
}
...
IPluginV2IOExt* ChunkPluginCreator::createPlugin(
...
IPluginV2IOExt* ChunkPluginCreator::deserializePlugin(const char* name,

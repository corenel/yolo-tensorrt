...
Yolo::Yolo(const NetworkInfo& networkInfo, const InferParams& inferParams)
  ...
  if (m_NetworkType == yolo_trt::ModelType::YOLOV5)
    parse_cfg_blocks_v5(m_configBlocks);
  else
    parseConfigBlocks();

  m_EnginePath = networkInfo.data_path + "-" + m_Precision + "-batch" +
...
/// \return
///
std::vector<int> split_layer_index(const std::string& s_,
...
void Yolo::createYOLOEngine(const nvinfer1::DataType dataType,
  ...
  nvinfer1::IConstantLayer* constDivide =
      m_Network->addConstant(divDims, divWeights);
  ...
  for (uint32_t i = 0; i < m_configBlocks.size(); ++i) {
    ...
    if (m_configBlocks.at(i).at("type") == "net") {
      ...
      if (m_configBlocks.at(i).find("activation") !=
      ...
      if ((m_configBlocks.at(i).find("batch_normalize") !=
           m_configBlocks.at(i).end()) &&
          ("leaky" == activation)) {
        out = netAddConvBNLeaky(i, m_configBlocks.at(i), weights, trtWeights,
                                weightPtr, channels, previous, m_Network);
        layerType = "conv-bn-leaky";
      } else if ((m_configBlocks.at(i).find("batch_normalize") !=
                  m_configBlocks.at(i).end()) &&
                 ("mish" == activation)) {
        out = net_conv_bn_mish(i, m_configBlocks.at(i), weights, trtWeights,
                               weightPtr, channels, previous, m_Network);
        layerType = "conv-bn-mish";
      } else  // if("linear" == activation)
      {
        out = netAddConvLinear(i, m_configBlocks.at(i), weights, trtWeights,
                               weightPtr, channels, previous, m_Network);
      ...
      printLayerInfo(layerIndex, layerType, inputVol, outputVol,
      ...
      nvinfer1::IPluginV2* yoloPlugin = new nvinfer1::YoloLayer(
          m_OutputTensors.at(outputTensorCount).numBBoxes,
      ...
      nvinfer1::IPluginV2Layer* yolo =
          m_Network->addPluginV2(&previous, 1, *yoloPlugin);
      ...
      printLayerInfo(layerIndex, "yolo", inputVol, outputVol,
                     std::to_string(weightPtr));
      ++outputTensorCount;
      ...
      if (found != std::string::npos)  // concate multi layers
      {
        std::vector<int> vec_index =
            split_layer_index(m_configBlocks.at(i).at("layers"), ",");
        for (auto& ind_layer : vec_index) {
          if (ind_layer < 0)
            ind_layer = static_cast<int>(tensorOutputs.size()) + ind_layer;
          assert(ind_layer < static_cast<int>(tensorOutputs.size()) &&
                 ind_layer >= 0);
        }
        nvinfer1::ITensor** concatInputs =
            reinterpret_cast<nvinfer1::ITensor**>(
        ...
        nvinfer1::IConcatenationLayer* concat = m_Network->addConcatenation(
            concatInputs, static_cast<int>(vec_index.size()));
      ...
      } else  // single layer
      {
        ...
        if (idx < 0) idx = static_cast<int>(tensorOutputs.size()) + idx;
        ...
        // yolov4-tiny route split layer
          ...
          if (m_configBlocks.at(i).find("group_id") ==
              m_configBlocks.at(i).end())
            assert(0);
          ...
          printLayerInfo(layerIndex, "chunk", inputVol, outputVol,
                         std::to_string(weightPtr));
        }
      }
      ...
      std::string inputVol = dimsToString(previous->getDimensions());
      nvinfer1::ILayer* out =
          netAddUpsample(i - 1, m_configBlocks[i], weights, trtWeights,
      ...
      std::string inputVol = dimsToString(previous->getDimensions());
      nvinfer1::ILayer* out =
          netAddMaxpool(i, m_configBlocks.at(i), previous, m_Network);
      previous = out->getOutput(0);
      assert(previous != nullptr);
      std::string outputVol = dimsToString(previous->getDimensions());
      tensorOutputs.push_back(out->getOutput(0));
  ...
  if (static_cast<int>(weights.size()) != weightPtr) {
    std::cout << "Number of unused weights left : "
              << static_cast<int>(weights.size()) - weightPtr << std::endl;
    assert(0);
  }

  //   std::cout << "Output blob names :" << std::endl;
  //   for (auto& tensor : m_OutputTensors) std::cout << tensor.blobName <<
  //   std::endl;

  // Create and cache the engine if not already present
  if (fileExists(m_EnginePath)) {
    std::cout << "Using previously generated plan file located at "
              << m_EnginePath << std::endl;
    destroyNetworkUtils(trtWeights);
    return;
  }

  // std::cout << "Unable to find cached TensorRT engine for network : " <<
  // m_NetworkType << " precision : " << m_Precision << " and batch size :" <<
  ...
  size_t workspaceSize =
      config->getMemoryPoolLimit(nvinfer1::MemoryPoolType::kWORKSPACE);
  size_t dlaManagedSRAMSize =
      config->getMemoryPoolLimit(nvinfer1::MemoryPoolType::kDLA_MANAGED_SRAM);
  size_t dlaLocalDRAMSize =
      config->getMemoryPoolLimit(nvinfer1::MemoryPoolType::kDLA_LOCAL_DRAM);
  size_t dlaGlobalDRAMSize =
      config->getMemoryPoolLimit(nvinfer1::MemoryPoolType::kDLA_GLOBAL_DRAM);
  std::cout << "workspaceSize = " << workspaceSize
            << ", dlaManagedSRAMSize = " << dlaManagedSRAMSize
            << ", dlaLocalDRAMSize = " << dlaLocalDRAMSize
            << ", dlaGlobalDRAMSize = " << dlaGlobalDRAMSize << std::endl;

  config->setMemoryPoolLimit(nvinfer1::MemoryPoolType::kWORKSPACE,
                             m_videoMemory ? m_videoMemory : (1 << 20));
#endif
...
void parse_c3_args(const std::string s_args_, int& n_out_ch_,
                   bool& b_shourt_cut_) {
...
void parse_bottleneck_args(const std::string s_args_, int& n_out_ch_,
                           bool& b_shourt_cut_) {
  std::string s_args = s_args_;
  while (!s_args.empty()) {
    auto npos = s_args.find_first_of(',');
...
void parse_spp_args(const std::string s_args_, int& n_filters_,
                    std::vector<int>& vec_k_) {
  std::string s_args = s_args_;
  vec_k_.clear();
  size_t pos = 0;
  std::string token;
...
void Yolo::create_engine_yolov5(const nvinfer1::DataType dataType,
  ...
  for (uint32_t i = 0; i < m_configBlocks.size(); ++i) {
    ...
    if ("net" == m_configBlocks.at(i).at("type")) {
      ...
      filters = (n_output != filters)
                    ? make_division(filters * _f_width_multiple, 8)
                    : filters;
      nvinfer1::ILayer* out =
          layer_focus(trtWeights, "model." + std::to_string(i - 1), model_wts,
                      previous, filters, kernel_size, trtWeights, m_Network);
      previous = out->getOutput(0);
      assert(previous != nullptr);
      channels = getNumChannels(previous);
      std::string outputVol = dimsToString(previous->getDimensions());
      ...
      int kernel_size = args[1];
      int stride = args[2];
      int n_out_channel = (n_output != filters)
                              ? make_division(filters * _f_width_multiple, 8)
                              : filters;
      nvinfer1::ILayer* out = layer_conv_bn_act(
          trtWeights, "model." + std::to_string(i - 1), model_wts, previous,
          m_Network, n_out_channel, kernel_size, stride);
      previous = out->getOutput(0);
      assert(previous != nullptr);
      channels = getNumChannels(previous);
      std::string outputVol = dimsToString(previous->getDimensions());
      ...
      int n_out_channel = (n_output != filters)
                              ? make_division(filters * _f_width_multiple, 8)
                              : filters;
      int n_depth = (number > 1)
                        ? (std::max(int(round(_f_depth_multiple * number)), 1))
                        : number;
      std::string s_model_name = "model." + std::to_string(i - 1);
      auto out = C3(trtWeights, s_model_name, model_wts, m_Network, previous,
                    n_out_channel, n_depth, short_cut);
      previous = out->getOutput(0);
      assert(previous != nullptr);
      channels = getNumChannels(previous);
      std::string outputVol = dimsToString(previous->getDimensions());
      ...
      std::string s_model_name = "model." + std::to_string(i - 1);
      auto out =
          layer_bottleneck_csp(trtWeights, s_model_name, model_wts, m_Network,
                               previous, n_out_channel, n_depth, short_cut);
      previous = out->getOutput(0);
      assert(previous != nullptr);
      channels = getNumChannels(previous);
      std::string outputVol = dimsToString(previous->getDimensions());
      ...
      int n_out_channel = (n_output != filters)
                              ...
                              : filters;
      std::string s_model_name = "model." + std::to_string(i - 1);
      auto out = layer_spp(trtWeights, s_model_name, model_wts, m_Network,
                           previous, n_out_channel, vec_k);
      previous = out->getOutput(0);
      assert(previous != nullptr);
      channels = getNumChannels(previous);
      std::string outputVol = dimsToString(previous->getDimensions());
      ...
      int n_out_channel = (n_output != filters)
                              ...
                              : filters;
      std::string s_model_name = "model." + std::to_string(i - 1);
      auto out = layer_sppf(trtWeights, s_model_name, model_wts, m_Network,
                            previous, n_out_channel, args[1]);
      previous = out->getOutput(0);
      assert(previous != nullptr);
      channels = getNumChannels(previous);
      std::string outputVol = dimsToString(previous->getDimensions());
      ...
      std::string s_model_name = "model." + std::to_string(i - 1);
      auto out =
          layer_upsample(s_model_name, model_wts, m_Network, previous, scale);
      previous = out->getOutput(0);
      assert(previous != nullptr);
      channels = getNumChannels(previous);
      std::string outputVol = dimsToString(previous->getDimensions());
      ...
      nvinfer1::ITensor** concat_tensor = reinterpret_cast<nvinfer1::ITensor**>(
          malloc(sizeof(nvinfer1::ITensor*) * vec_from.size()));
      for (size_t j = 0; j < vec_from.size(); ++j) {
        concat_tensor[j] = tensorOutputs[vec_from[j]];
      }
      nvinfer1::IConcatenationLayer* concat = m_Network->addConcatenation(
          concat_tensor, static_cast<int>(vec_from.size()));
      assert(concat != nullptr);
      ...
      for (size_t ind_from = 0; ind_from < vec_from.size(); ++ind_from) {
        ...
        curYoloTensor.stride_h = m_InputH / curYoloTensor.grid_h;
        curYoloTensor.stride_w = m_InputW / curYoloTensor.grid_w;
        m_OutputTensors.at(ind_from).volume =
            curYoloTensor.grid_h * curYoloTensor.grid_w *
            (curYoloTensor.numBBoxes * (5 + curYoloTensor.numClasses));
        std::string layerName = "yolo_" + std::to_string(ind_from);
        curYoloTensor.blobName = layerName;
        /*auto creator = getPluginRegistry()->getPluginCreator("DETECT_TRT",
        "1.0"); const nvinfer1::PluginFieldCollection* pluginData =
        creator->getFieldNames(); nvinfer1::IPluginV2 *yoloPlugin =
        creator->createPlugin(("detect" + std::to_string(ind_from)).c_str(),
        pluginData);*/
        nvinfer1::IPluginV2* yoloPlugin = new nvinfer1::Detect(
            curYoloTensor.numBBoxes, curYoloTensor.numClasses,
            curYoloTensor.grid_h, curYoloTensor.grid_w);
        assert(yoloPlugin != nullptr);
        ...
        tensorOutputs.push_back(yolo->getOutput(0));
        printLayerInfo(layerIndex, "detect" + std::to_string(ind_from),
                       inputVol, outputVol, "");
      }
    }  // end detect
    else {
...
void Yolo::load_weights_v5(
    const std::string s_weights_path_,
...
void Yolo::doInference(const unsigned char* input, const uint32_t batchSize) {
  ...
  NV_CUDA_CHECK(cudaMemcpyAsync(m_DeviceBuffers.at(m_InputBindingIndex), input,
                                batchSize * m_InputSize * sizeof(float),
  ...
  for (auto& tensor : m_OutputTensors) {
    NV_CUDA_CHECK(cudaMemcpyAsync(tensor.hostBuffer,
                                  m_DeviceBuffers.at(tensor.bindingIndex),
...
std::vector<BBoxInfo> Yolo::decodeDetections(const int& imageIdx,
  ...
  for (auto& tensor : m_OutputTensors) {
    std::vector<BBoxInfo> curBInfo =
        decodeTensor(imageIdx, imageH, imageW, tensor);
    binfo.insert(binfo.end(), curBInfo.begin(), curBInfo.end());
  }
  return binfo;
}

std::vector<std::map<std::string, std::string>> Yolo::parseConfigFile(
...
void Yolo::parseConfigBlocks() {
  for (auto block : m_configBlocks) {
    if (block.at("type") == "net") {
      ...
      if (m_BatchSize < 1) m_BatchSize = std::stoi(trim(block.at("batch")));
      ...
      if ((m_NetworkType == yolo_trt::ModelType::YOLOV3) ||
          (m_NetworkType == yolo_trt::ModelType::YOLOV4) ||
          (m_NetworkType == yolo_trt::ModelType::YOLOV4_TINY)) {
        assert((block.find("mask") != block.end()) &&
               std::string("Missing 'mask' param in " + block.at("type") +
                           " layer")
                   .c_str());
      ...
      if (m_NetworkType == yolo_trt::ModelType::YOLOV4)  // pan
      {
      ...
      outputTensor.volume =
          outputTensor.grid_h * outputTensor.grid_w *
...
void Yolo::parse_cfg_blocks_v5(
    const std::vector<std::map<std::string, std::string>>& vec_block_) {
    ...
    if ("net" == block.at("type")) {
      ...
      for (uint32_t i = 0; i < vec_from.size(); ++i) {
        ...
        outputTensor.numBBoxes =
            static_cast<uint32_t>(outputTensor.masks.size());
...
void Yolo::allocateBuffers() {
  ...
  for (auto& tensor : m_OutputTensors) {
    ...
    NV_CUDA_CHECK(cudaMallocHost(&tensor.hostBuffer,
                                 tensor.volume * m_BatchSize * sizeof(float)));
  }
}
...
bool Yolo::verifyYoloEngine() {
  ...
  for (const auto& tensor : m_OutputTensors) {
    assert(!strcmp(m_Engine->getBindingName(tensor.bindingIndex),
                   tensor.blobName.c_str()) &&
           "Blobs names dont match between cfg and engine file \n");
    assert(get3DTensorVolume(m_Engine->getBindingDimensions(
               tensor.bindingIndex)) == tensor.volume &&
           "Tensor volumes dont match between cfg and engine file \n");
  }

  assert(m_Engine->bindingIsInput(m_InputBindingIndex) &&
         "Incorrect input binding index \n");
...
void Yolo::writePlanFileToDisk() {
  ...
  gieModelStream.write(static_cast<const char*>(m_ModelStream->data()),
                       m_ModelStream->size());

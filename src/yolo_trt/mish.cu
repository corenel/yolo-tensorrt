...
size_t MishPlugin::getSerializationSize() const { return sizeof(input_size_); }

int MishPlugin::initialize() { return 0; }

Dims MishPlugin::getOutputDimensions(int index, const Dims* inputs,
                                     int nbInputDims) {
...
DataType MishPlugin::getOutputDataType(int index,
...
bool MishPlugin::isOutputBroadcastAcrossBatch(int outputIndex,
...
void MishPlugin::configurePlugin(const PluginTensorDesc* in, int nbInput,
...
void MishPlugin::attachToContext(cudnnContext* cudnnContext,
...
void MishPlugin::forwardGpu(const float* const* inputs, float* output,
...
IPluginV2IOExt* MishPluginCreator::createPlugin(
...
IPluginV2IOExt* MishPluginCreator::deserializePlugin(const char* name,

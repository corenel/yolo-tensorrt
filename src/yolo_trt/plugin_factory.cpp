...
bool YoloLayer::supportsFormat(DataType type,
...
YoloLayer::YoloLayer(const uint32_t& numBoxes, const uint32_t& numClasses,
...
nvinfer1::Dims YoloLayer::getOutputDimensions(int index,
...
size_t YoloLayer::getSerializationSize() const noexcept {
  return sizeof(m_NumBoxes) + sizeof(m_NumClasses) + sizeof(_n_grid_w) +
         sizeof(_n_grid_h) + sizeof(m_OutputSize);
}
...
void YoloLayerPluginCreator::setPluginNamespace(

...
struct Result {
  ...
  Result(int id, float prob, const cv::Rect& brect)
    ...
    m_rrect = cv::RotatedRect(
        m_brect.tl(),
        cv::Point2f(static_cast<float>(m_brect.x + m_brect.width),
                    static_cast<float>(m_brect.y)),
        m_brect.br());
    if (m_rrect.size.width < 1) m_rrect.size.width = 1;
    if (m_rrect.size.height < 1) m_rrect.size.height = 1;
  }
